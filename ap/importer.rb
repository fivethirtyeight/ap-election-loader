  ...
  private
    ...
    def initialize_state(state_abbr)
      ...
      q <<-eos
        ...
        select stage_ap_results.test_flag, concat(date_format(stage_ap_races.election_date, '%y%m'), stage_ap_results.race_county_id), candidate_id, party, incumbent, vote_count, winner, natl_order
      ...
      q <<-eos
        ...
        select test_flag, concat(date_format(election_date, '%y%m'), race_county_id), race_number, election_date, state_postal, county_number, fips_code, county_name, office_id, race_type_id, seat_number, office_name, seat_name, race_type_party, race_type, office_description, number_of_winners, number_in_runoff, precincts_reporting, total_precincts, now()
        from stage_ap_races
    ...
    def merge_state(state_abbr)
      ...
      q <<-eos
        update ap_results
          inner join stage_ap_races on ap_results.ap_race_id = concat(date_format(stage_ap_races.election_date, '%y%m'), stage_ap_races.race_county_id)
